...
from api.helpers import get_authorized_run, get_authorized_run_details
...
async def run_agent_task(
    ...
    try:
        ...
        try:
            await db_manager.update_run_status(run_id, "failed", error_message=str(e))
        except Exception:
...
async def get_result_details(
    ...
    Requires the 'view:results' permission.
    """
    details = await get_authorized_run_details(run_id, user, db_manager)
...
async def download_result(
    ...
    """
    details = await get_authorized_run_details(
        run_id, user, db_manager, action="download"
    )
