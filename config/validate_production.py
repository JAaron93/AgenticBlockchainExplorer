...
def validate_production_config(config_path: str = "./config.json") -> Tuple[bool, List[str]]:
    ...
    if not isinstance(secret_key, str):
        issues.append("❌ app.secret_key must be a string")
    elif len(secret_key) < 32:
        issues.append("❌ app.secret_key must be at least 32 characters")
    elif "example" in secret_key.lower() or "change" in secret_key.lower():
    ...
    if not isinstance(cors_origins, list):
        issues.append("❌ cors.allowed_origins must be a list")
    else:
    ...
    if isinstance(callback_url, str) and "localhost" in callback_url:
        issues.append("❌ auth0.callback_url contains localhost")
    if isinstance(logout_url, str) and "localhost" in logout_url:
        issues.append("❌ auth0.logout_url contains localhost")
    
    if isinstance(callback_url, str) and callback_url and not callback_url.startswith("https://"):
        issues.append("⚠️  auth0.callback_url should use HTTPS")
    if isinstance(logout_url, str) and logout_url and not logout_url.startswith("https://"):
        issues.append("⚠️  auth0.logout_url should use HTTPS")
    
    # Check database URL
    db_url = config.get("database", {}).get("url", "")
    if isinstance(db_url, str) and ("<USER>" in db_url or "<PASSWORD>" in db_url or "<DB_HOST>" in db_url):
        issues.append("❌ database.url contains placeholders - replace with actual values")
    if isinstance(db_url, str) and "user:password" in db_url:
