...
def _get_blockchain_validator():
    ...
    if _blockchain_validator is None:
        try:
            ...
            logger.debug("BlockchainDataValidator initialized for EtherscanCollector")
...
class EtherscanCollector(ExplorerCollector):
    ...
    async def fetch_stablecoin_transactions(
        ...
        """Fetch stablecoin transactions from Etherscan.
        ...
        # Etherscan API parameters for token transfers
        # We use the tokentx endpoint which returns ERC-20 transfers
        params = {
            "module": "account",
            "action": "tokentx",
            "contractaddress": contract_address,
            "page": 1,
            "offset": min(limit, 10000),  # Etherscan max is 10000 per request
    ...
    def _parse_holder(
        ...
        try:
            ...
            # We'll use current time as a placeholder
            ...
            return Holder(
                ...
                stablecoin=stablecoin,
                chain=self.chain,
                first_seen=now,  # Would need historical data to determine
                last_activity=now,  # Would need transaction history to determine
    ...
    async def fetch_token_holders(
        ...
        Note: The token holder list endpoint requires a Pro API key on Etherscan.
        This method will return an empty list if the endpoint is not available.
        ...
        # Token holder list endpoint (requires Pro API)
        params = {
            "module": "token",
